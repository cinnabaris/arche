...
<dashboard-layout :title="title" :role="role" :init="init">
  <div style="float: right;">
    <el-button size="mini" type="primary" @click="()=>this.$router.push({name:'forum.posts.new'})" class="el-icon-plus" />
  </div>
    ...
    <el-table-column fixed="right" :label="$t('buttons.operator')" width="120">
      <template slot-scope="scope">
        <el-button-group>
          ...
          <el-button v-if="scope.row.editable" size="mini" type="danger" @click="handleDelete(scope.row.id)" class="el-icon-delete"/>
        </el-button-group>
      </template>
    </el-table-column>
  </el-table>
  ...
  methods: {
    ...
    init() {
      client().request(`query list{
        listForumPost{
          id, body, updatedAt, editable
        }
      }`, {}).then((rst) => {
        this.items = rst.listForumPost
      }).catch(failed)
    },
    setPage(p) {
      this.page = p
